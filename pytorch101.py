...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y = torch.zeros(x_tensor.shape[0], int(x_tensor.max().item()) + 1)
    ##########################################################################
    #                      TODO: Implement this function                     #
    ##########################################################################
    # Replace "pass" statement with your code
    y[torch.arange(x_tensor.shape[0]), x_tensor] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    mins = y.argmin(dim=1)
    y[torch.arange(y.shape[0]), mins] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    Returns:
        z: Tensor of shape (B, N, P) where z[i] of shape (N, P) is the result
            ...
            shape (M, P). The output z should have the same dtype as x.
    """
    z = torch.empty(x.shape[0], x.shape[1], y.shape[2], dtype=x.dtype, device=x.device)
    ###########################################################################
    #                      TODO: Implement this function                      #
    ###########################################################################
    # Replace "pass" statement with your code
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean = x.sum(dim=0).reshape(1, -1) / num_rows
    centered = x - mean
    std = ((centered ** 2).sum(dim=0).reshape(1, -1) / num_rows).sqrt()
    y = centered / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    groups = torch.repeat_interleave(
        torch.arange(ls.shape[0], device=ls.device), ls
    )
    y = torch.zeros(ls.shape[0], dtype=values.dtype, device=values.device)
    ##########################################################################
    # TODO: Implement this function without using `for` loops and store the  #
    # mean values as a tensor in `y`.                                        #
    ##########################################################################
    # Replace "pass" statement with your code
    y.scatter_add_(0, groups, values)
    y = y / ls.to(device=y.device, dtype=y.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    indices = torch.full_like(uniques, x.shape[0], dtype=torch.int64)
    ##########################################################################
